...
- La E2E completa pasa tras validar los cuatro perfiles, con 179 GLB adaptados por perfil,
  arranque offline, IndexedDB, bloqueo, historial y flujo de inventario. El arnés acepta
  estados de navegación `interactive`/`complete` y reintenta el archivo de puerto de Chrome
  cuando todavía está bloqueado durante el arranque.
- En el visor, los gestos táctiles en modo inventario no desplazan la cámara ni persisten
  un encuadre accidental; la pintura sigue pasando por la misma transacción reversible.
...
- Tafonomía: el registro científico ofrece un catálogo curado, anotaciones estructuradas
  múltiples por elemento y un campo explícito de evidencia/fuente; el detalle se conserva
  en copias y exportaciones mediante `Taphonomy_Detail`.
...
- Ampliación: se publican 537 GLB adaptados con Blender para femenino, infante y
  neonato, con metadatos y licencia incorporados; no se han validado científicamente.
...
- Cada registro óseo puede incluir ahora un `Specimen_ID` opcional. Se conserva en
  ...
  no tiene una asociación específica. La cobertura y los conflictos también se incluyen
  en el informe imprimible, con etiquetas localizadas para español e inglés. No resuelve
  por sí solo remontaje ni MNE/MNI.

- 179 elementos con geometría procedural propia para los cuatro perfiles (716
  combinaciones), actualizada a procedural-1.2.0. Además, los paquetes publicados
  de femenino, infante y neonato contienen 537 adaptaciones Blender derivadas del
  adulto masculino. Las dos capas son didácticas y no sustituyen validación científica.
...
| 6–9, 66 | Despliegue, slerp y mesa operativos. La disposición desplegada organiza ahora las piezas en bandas regionales separadas y conserva una banda de compatibilidad para registros sin región; la prueba procedural comprueba el orden de bandas con el catálogo completo. No se certifica ausencia de colisiones durante toda la transición ni articulación exacta de los GLB externos. |
...
| 14, 22, 83 | Comparación de perfiles esquemáticos e inventarios y ejercicios básicos existentes. La comparación de inventarios identifica y filtra por individuo, contexto, UE y campaña, coteja porciones independientes por elemento y conserva la diferencia entre valores cero y campos desconocidos (`—`). La vista agregada admite dos o más fuentes y la comparación 3D aplica escala relativa didáctica, sigue despliegue/disposición/filtros científicos, comparte cámara, permite seleccionar el mismo hueso desde cualquiera de las dos escenas y hereda transparencia, X-Ray, wireframe y colores por región. El aprendizaje identifica/localiza con niveles, feedback y puntuación de intentos, cubierto por E2E. Pendientes escala física validada, comparación por hueso más profunda y revisión pedagógica. |
| 23–34, 36–37, 41–57, 59–65, 67–73 | Inventario, estados, conservación, porciones, lateralidad, dientes FDI, fragmentos, peso, fotos, notas, filtros, pintura y tablas implementados. Conservación distingue no evaluada, no evaluable y porcentaje ausente; la normalización elimina porcentajes incompatibles con no evaluable y las medias excluyen desconocidos. E2E cubre odontograma permanente/deciduo, acciones masivas confirmadas, selección múltiple reversible, modos rápidos, filtros combinados de estado/lateralidad/conservación/tipo y sinónimos del buscador. No se certifican todos los casos de cada formulario ni un mapa científico completo. |
| 38–40, 97 | Métodos visibles y corrección del uso de fragmentos. NISP automático cuenta registros; MNE/MNI son mínimos provisionales según categorías determinadas y asociaciones introducidas. El MNI usa el máximo de individuos explícitos por combinación de elemento y lado, y el análisis resuelve las asociaciones jerárquicas explícitas de individuo/contexto antes de usar los campos heredados. Las revisiones manuales se etiquetan como vigentes u obsoletas según su firma de inventario y el panel muestra un aviso explícito cuando requieren actualización. Las medias de resumen, regionales y de conservación excluyen valores desconocidos; el mapa de fragmentación también separa fragmentos observados de elementos sin registro. Para colecciones mezcladas se necesita análisis especializado y revisión manual documentada. |
| 58, 69 | Deshacer/rehacer y registro de cambios mediante snapshots comunes para inventario, ficha científica, importación, fotografías, fragmentos indeterminados, componentes inmaduros, odontogramas, contexto, asociaciones jerárquicas, medidas, landmarks, calibraciones, filtros y configuración visual; queda revisión especializada de operaciones futuras. |
| 74–78 | Informe imprimible/PDF del navegador con reglas A4 para paginación, ficha de contexto, jerarquía normalizada, observaciones inmaduras, JSON, CSV multilínea, XLSX e importación con previsualización existentes. Todas las exportaciones CSV/XLSX protegen ahora texto introducido con prefijos de fórmula, recuperan porciones independientes normalizadas y conservan asociaciones jerárquicas por registro. XLSX incluye una hoja `Esquema` autodocumentada y una hoja de asociaciones jerárquicas importable, además de las hojas auxiliares; el informe usa ahora un mapa vectorial regional con silueta reconocible, patrones de estado y contadores de progreso. Un fixture de campo verifica el round-trip JSON/backup de individuos, especímenes, porciones, dientes, fragmentos, jerarquía y notas multilínea. Siguen pendientes casos exportados de colecciones reales, PDF de maquetación avanzada y validación científica de la cartografía. |
| 79–82, 98 | IndexedDB, proyectos, guardado y copias de datos locales. Las copias `.osteo3d` incluyen binarios, CRC y SHA-256; la CSP y `no-referrer` limitan salidas implícitas, y `validate-privacy` audita el código fuente en CI. La jerarquía normalizada conserva entidades, relaciones básicas y asociaciones explícitas por registro óseo, exportables/importables en CSV/XLSX y reversibles; queda una interfaz relacional completa para múltiples yacimientos/campañas y los binarios de modelos personalizados no van incluidos en la copia JSON. No eliminar la caché como sustituto de una copia completa. |
| 85, 90–92, 114–115 | ES/EN y búsqueda por latín existentes; acceso móvil a catálogo/perfil corregido; foco visible, diálogos con foco modal y movimiento reducido incorporados. El editor científico, incluidos detalles tafonómicos/patológicos y asociaciones jerárquicas, localiza sus etiquetas al inglés y queda cubierto por E2E. La E2E comprueba el inspector al 125 %, 150 % y 200 % sin overflow y con canvas sincronizado; queda validación de contraste, lector de pantalla, zoom y tamaños táctiles en dispositivos reales, además de RTL y algunos mensajes auxiliares. |
| 86–89, 100, 112–113 | PWA, descarga/caché por perfil, carga diferida e importación de modelos operativas; los esquemas están en el shell offline. Manifiesto y registro de fuentes usan una clave de esquema para evitar listas de modelos obsoletas en Service Worker. La calidad baja añade LOD geométrico conservador y acotado para no bloquear la carga completa; la carga GLB limita decodificadores concurrentes a 2/4 y cede el hilo tras cada asset para conservar respuesta del render. La actualización protege además los borradores del informe: no recarga si hay campos sin guardar y lo verifica en navegador. Quedan validación visual del LOD, optimización/medición sostenida en móviles reales y validación de todos los formatos personalizados. |
| 93–94, 108–109, 116 | Separación nueva del generador, análisis y cargador; preservación de IDs y datos. Los exportadores XLSX/pesos, los controles de fotografías, los filtros extendidos y el reset de nuevo proyecto se enlazan sin temporizadores de carga arbitrarios; el diagnóstico de capacidades se inicializa una sola vez y el gestor de proyectos usa una microtarea segura. La sincronización de mesa, ocultación, transparencia e iluminación responde a eventos en lugar de sondeo continuo; los ciclos periódicos restantes se omiten en segundo plano, pero `main.js` conserva deuda de modularidad y algunos refrescos históricos. |
