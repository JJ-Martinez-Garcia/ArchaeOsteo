...
const importedModels = await importModelPackageFiles('adult_male', [{ name: 'skull.glb', size: 4, arrayBuffer: async () => new ArrayBuffer(4) }, { name: 'notes.txt', size: 1, arrayBuffer: async () => new ArrayBuffer(1) }], ['skull']);
assert.equal(importedModels.importedCount, 1);
assert.equal(importedModels.rejectedCount, 1);
