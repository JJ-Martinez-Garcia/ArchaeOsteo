...
import { cacheCustomModelFile, customModelUrl, downloadModelPackage, formatPackageSize, getCachedModelBoneIds, glbContainsBoneId, importModelPackageFiles, MAX_LOCAL_MODEL_BYTES, modelPackageDownloadPlan, modelPackageSummary, validateModelManifest, validateModelSourceRegistry } from '../src/anatomy/package.js';
...
import { deriveHierarchy, hierarchyCounts, normalizeHierarchy } from '../src/domain/hierarchy.js';
import { mergeAuxiliaryXlsx } from '../src/ui/extended.js';

const text = async path => readFile(path, 'utf8');
const exists = async path => { try { await access(path); return true; } catch { return false; } };
const e2eScript = await text('scripts/e2e-pwa.mjs');
assert.match(e2eScript, /async function withTimeout\(promise, timeout, label\)/);
assert.match(e2eScript, /withTimeout\(client\.send\(method, params\), timeout, `CDP \$\{method\}`\)/);

const manifest = JSON.parse(await text('public/manifest.json'));
const projectSchema = JSON.parse(await text('public/schemas/osteo3d-project-v3.schema.json'));
assert.equal(projectSchema.properties.schemaVersion.const, 3);
assert.equal(projectSchema.additionalProperties, false);
assert.ok(projectSchema.required.includes('profile'));
assert.match(await text('src/data/project-schema.js'), /public\/schemas\/osteo3d-project-v3\.schema\.json/);
const packageJson = JSON.parse(await text('package.json'));
const indexHtml = await text('index.html');
assert.equal(await exists('PWA_STATUS.md'), true);
assert.equal(await exists('CHANGELOG.md'), true);
assert.match(await text('CHANGELOG.md'), /Unreleased/);
...
assert.match(main, /function reportFormHasUnsavedChanges/);
assert.match(main, /Guarda el informe antes de actualizar/);
...
assert.match(extendedUi, /const activeProject = \{ \.\.\.state, id: state\.projectId, projectName: state\.projectName \}/);
assert.match(extendedUi, /id === state\.projectId \? activeProject : loadProject\(id\)/);
assert.match(extendedUi, /No se pudieron leer los proyectos locales/);
...
assert.match(main, /hierarchyRefs:state\.hierarchyRefs/);
...
const malformedReport = normalizeProject({ report: { site: { unsafe: true }, campaign: 2026, individual: ['IND-1'] } });
assert.equal(malformedReport.report.site, '[object Object]');
assert.equal(malformedReport.report.campaign, '2026');
assert.equal(malformedReport.report.individual, 'IND-1');
...
assert.equal(normalizeHierarchy({ sites: [{ id: 'site:a', name: 'A' }], contexts: [{ id: 'context:c', name: 'C', parentId: 'site:no-existe' }] }).contexts[0].parentId, '');
...
const notEvaluable = normalizeProject({ preservation: { skull: 'not_evaluable' }, completeness: { skull: 0, mandible: 25 } });
assert.equal(notEvaluable.preservation.skull, 'not_evaluable');
assert.equal(notEvaluable.completeness.skull, undefined);
assert.equal(notEvaluable.completeness.mandible, 25);
...
const explicitLaterality = normalizeProject({ filters: { side: 'not_applicable' } });
assert.equal(explicitLaterality.filters.side, 'not_applicable');
...
assert.match(main, /Mapa esquelético de revisión/);
assert.match(main, /skeleton-map-region/);
assert.match(main, /map-mixed/);
...
assert.match(main, /matchesFilters\(layout\).*pendingHidden/);
...
assert.match(main, /modelLoadDurationMs/);
assert.match(main, /dataset\.modelLoadDurationMs/);
assert.match(main, /dataset\.fps/);
...
assert.match(main, /const modelLoadConcurrency = lowMemory \? 2 : 4/);
assert.match(main, /state\.inventoryMode && e\.pointerType === 'touch'.*e\.preventDefault\(\)/);
assert.match(main, /if \(!state\.inventoryMode && moved\) persistCameraView\(\)/);
...
assert.match(main, /function focusRadiusForBone/);
assert.match(main, /orbit\.radius=focusRadiusForBone\(selected\)/);
assert.match(main, /Math\.max\(\.55,Math\.min\(12/);
assert.match(main, /metadata\.adapted_from==='adult_male'/);
assert.match(main, /loadedSource==='glb-adapted'/);
assert.match(main, /function refreshModelProvenanceLabel/);
...
assert.match(main, /const categoryLabels=english\?\{craniometric:'Craniometric',osteometric:'Osteometric',pelvimetric:'Pelvimetric',dental:'Dental',pathology:'Pathology \/ trauma',taphonomy:'Taphonomy'\}/);
...
assert.match(main, /data-photo-name/);
assert.match(main, /commitInventoryEdit\('photo_edit'/);
assert.match(main, /aria-label=\"\$\{state\.language==='en'\?'Remove photo':'Eliminar fotografía'\}\"/);
...
assert.match(main, /pending\.forEach\(b=>\{state\.status\[b\.id\]='absent';\}\)/);
assert.match(main, /function clearBoneAssignment\(id\)/);
assert.match(main, /'completeness','fragments','weights','weightUnits','portions','portionRecords'/);
assert.match(main, /'developmentRecords','hierarchyRefs','individuals','specimens','ue','taphonomy','pathology','taphonomyDetails','pathologyDetails','notes'/);
...
assert.match(main, /data-paint-tool="percentage"/);
assert.match(main, /data-paint-tool="taphonomy"/);
assert.match(main, /data-paint-tool="erase"/);
assert.match(main, /data-quick-completeness="100"/);
assert.match(main, /data-quick-completeness="20"/);
assert.match(main, /data-clear-completeness/);
...
assert.match(await text('src/ui/extended.js'), /creativecommons\.org\/public-domain\/cc0/);
assert.match(await text('src/ui/extended.js'), /opensource\.org\/license\/mit/);
assert.match(await text('src/ui/extended.js'), /noopener noreferrer/);
assert.match(await text('src/ui/extended.js'), /Sources and licenses/);
assert.match(await text('src/ui/extended.js'), /sourcePanelMarkup/);
assert.match(await text('src/inspector-layout.css'), /html, body \{ overflow-x: hidden; \}/);
...
assert.match(await text('src/anatomy/package.js'), /replacedEntries = \[\]/);
...
assert.match(main, /loadedModelObjects\.set\(bone\.id,model\); registerLandmarkModelReference\(bone\.id, modelSource\); if\(bone\.id===state\.selected\)\{refreshProfileDetailLabel\(\);refreshModelProvenanceLabel\(\);\}/);
...
assert.match(main, /const width=renderer\.domElement\.clientWidth,height=renderer\.domElement\.clientHeight,placed=\[\]/);
assert.match(main, /!placed\.some\(other=>rect\.left<other\.right/);
assert.match(main, /bone-label-guide/);
assert.match(await text('src/styles.css'), /\.bone-label-guide\{position:absolute/);
...
assert.match(main, /const duplicateMandible=b\.id==='mandible'&&!state\.isolate/);
assert.match(main, /!duplicateMandible/);
...
assert.match(main, /const roots=\[\.\.\.group\.children\.filter\(object=>object\.visible\),\.\.\.\(comparisonGroup\?\.children\|\|\[\]\)\.filter\(object=>object\.visible\)\]/);
assert.match(main, /comparison:true/);
assert.match(main, /comparisonGroup\?\.traverse/);
assert.match(main, /comparisonOpacity/);
...
assert.match(await text('src/ui/extended.js'), /const sourceLabels = projects\.map/);
assert.match(await text('src/ui/extended.js'), /\.\.\.sourceLabels, \.\.\.statuses/);
...
assert.match(main, /selection-announcement/);
assert.match(main, /oste3d:bone-selected.*statusNamesEn/);
...
assert.match(main, /data-row-status.*paintBone\(id\)/);
...
assert.match(main, /renderPendingAwareList/);
assert.match(main, /state\.pendingOnly = !state\.pendingOnly; renderList\(\); updateInventory\(\)/);
...
assert.match(await text('src/domain/inventory-history.js'), /'hidden'.*'lightingMode'/s);
...
assert.ok(modelManifest.required_metadata.includes('license_url'));
...
assert.deepEqual(sourceRegistry.profiles.adult_male.license_urls, ['https://creativecommons.org/public-domain/cc0/', 'https://creativecommons.org/licenses/by-sa/4.0/']);
...
assert.equal(validateModelSourceRegistry({ profiles: { adult_male: { asset_status: 'partial' } } }, { schema_version: 1, profiles: { adult_male: { source_status: 'published', author: 'A', institution: 'I', url: 'javascript:alert(1)', license: 'MIT', version: '1', consulted_at: '2026-01-01' } } }).valid, false);
assert.equal(validateModelSourceRegistry({ profiles: { adult_male: { asset_status: 'partial' } } }, { schema_version: 1, profiles: { adult_male: { source_status: 'published', author: 'A', institution: 'I', url: 'https://example.org', license: 'MIT', version: '1', consulted_at: '01/01/2026' } } }).valid, false);
assert.equal(validateModelSourceRegistry({ profiles: { adult_male: { asset_status: 'partial' } } }, { schema_version: 1, profiles: { adult_male: { source_status: 'published', author: 'A', institution: 'I', url: 'https://example.org', license: 'MIT', version: '1', consulted_at: '2026-02-31' } } }).valid, false);
assert.equal(validateModelSourceRegistry({ profiles: { adult_male: { asset_status: 'partial' } } }, { schema_version: 1, profiles: { adult_male: { source_status: 'published', author: 'A', institution: 'I', url: 'https://example.org', license: 'MIT', license_url: 'javascript:alert(1)', version: '1', consulted_at: '2026-01-01' } } }).valid, false);
assert.equal(validateModelSourceRegistry({ profiles: { adult_male: { asset_status: 'partial' } } }, { schema_version: 1, profiles: { adult_male: { source_status: 'published', author: 'A', institution: 'I', url: 'https://example.org', license: 'MIT', license_url: 'https://example.org', license_urls: ['javascript:alert(1)'], version: '1', consulted_at: '2026-01-01' } } }).valid, false);
...
assert.match(main, /clearBoneAssignment\(id\)/);
assert.match(main, /clearBoneAssignment\(id\)/);
...
assert.match(await text('src/styles.css'), /forced-colors: active/);
assert.match(await text('src/styles.css'), /prefers-contrast: more/);
assert.match(main, /importCustomModel = async file =>/);
assert.match(main, /file\.size\)>MAX_LOCAL_MODEL_BYTES/);
assert.match(main, /oste3d:custom-models-changed/);
assert.match(main, /aria-pressed',String\(state\.skeletonFilter/);
assert.match(extendedUi, /cached: false/);
...
assert.match(main, /start-surface-pathology/);
assert.match(main, /record-surface-pathology/);
assert.match(main, /category\s*===\s*'pathology'/);
...
assert.match(main, /if \(state\.inventoryMode\) return; if \(dragState\.shift/);
...
assert.doesNotMatch(main, /setTimeout\(\(\) => installCapabilityDiagnostics/);
assert.match(main, /queueMicrotask\(\(\) => initProjectManager/);
...
const oversizedImport = await importModelPackageFiles('adult_male', [{ name: 'skull.glb', size: MAX_LOCAL_MODEL_BYTES + 1, arrayBuffer: async () => { oversizedRead = true; return makeGlb(['skull']); } }], ['skull']);
assert.equal(oversizedImport.importedCount, 0);
assert.equal(oversizedImport.rejectedCount, 1);
...
const stableCaches = globalThis.caches;
globalThis.caches = { open: async () => ({
  match: async key => modelCacheEntries.get(key) || undefined,
  put: async (key, response) => { if (key === customModelUrl('adult_male', 'skull', 'gltf')) throw new Error('simulated cache failure'); modelCacheEntries.set(key, response); },
  delete: async key => modelCacheEntries.delete(key)
}) };
await assert.rejects(() => cacheCustomModelFile('adult_male', 'skull', { name: 'skull.gltf', size: 64, arrayBuffer: async () => makeGlb(['skull']) }), /simulated cache failure/);
assert.equal(modelCacheEntries.has(customModelUrl('adult_male', 'skull', 'glb')), true);
assert.equal(modelCacheEntries.has(customModelUrl('adult_male', 'skull', 'gltf')), false);
globalThis.caches = stableCaches;
...
const importedPortions = applyInventoryRows({}, [{ Bone_ID: 'left_femur', Portion_records: JSON.stringify({ shaft_mid: { status: 'fragmentary', completeness: 60, fragments: 2 } }) }], testBones);
assert.equal(importedPortions.portionRecords.left_femur.shaft_mid.completeness, 60);
assert.equal(importedPortions.portionRecords.left_femur.shaft_mid.fragments, 2);
