...
import { downloadModelPackage, formatPackageSize, importModelPackageFiles, modelPackageDownloadPlan, modelPackageSummary, validateModelManifest, validateModelSourceRegistry } from '../src/anatomy/package.js';
...
const importedModels = await importModelPackageFiles('adult_male', [{ name: 'skull.glb', size: 4, arrayBuffer: async () => new Uint8Array([0x67, 0x6c, 0x54, 0x46]).buffer }, { name: 'bad.glb', size: 4, arrayBuffer: async () => new ArrayBuffer(4) }, { name: 'notes.txt', size: 1, arrayBuffer: async () => new ArrayBuffer(1) }], ['skull', 'bad']);
