...
export function initExtendedFeatures({ state, bones, saveLocal, selectBone, renderList, renderStats, downloadFile, listProjects, loadProject, commitInventoryEdit, applyProjectData }) {
  ...
  document.querySelector('#multi-compare-panel-button').onclick = async () => {
    ...
    document.querySelector('#run-multi-compare').onclick = async () => {
      ...
      setTimeout(() => { if (!result) return; result.insertAdjacentHTML('beforeend', `<button id="export-multi-compare" class="secondary-action">${en ? 'Download CSV' : 'Descargar CSV'}</button>`); document.querySelector('#export-multi-compare').onclick = () => { const fields = ['Bone_ID', ...statuses]; const csv = [fields.join(','), ...rows.map(row => [row.bone.id, ...statuses.map(status => row.counts[status])].join(','))].join('\n'); downloadFile('osteo3d-multi-comparison.csv', csv, 'text/csv;charset=utf-8'); }; }, 0);
