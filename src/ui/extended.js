...
import { HIERARCHY_LEVELS, compareHierarchyMetrics, deriveHierarchy, hierarchyId, hierarchyInventoryMetrics, hierarchyRecordCounts, hierarchyTree, normalizeHierarchy } from '../domain/hierarchy.js';
...
export function mergeAuxiliaryXlsx(value, workbook, XLSX, bones) {
  ...
  const cameraView = cameraValues.length === 6 && cameraValues.every(Number.isFinite) ? { theta: cameraValues[0], phi: Math.max(.12, Math.min(Math.PI - .12, cameraValues[1])), radius: Math.max(.35, Math.min(40, cameraValues[2])), target: cameraValues.slice(3) } : value.cameraView;
  const hasDental3dFlag = visual && Object.prototype.hasOwnProperty.call(visual, 'Dental_3D');
  const dental3d = hasDental3dFlag ? visual.Dental_3D === true || String(visual.Dental_3D).toLowerCase() === 'true' : Boolean(value.dental3d);
  ...
  const hierarchyRefs = { ...(value.hierarchyRefs || {}) };
  for (const row of rows('Asociaciones jerárquicas')) if (validIds.has(row.Bone_ID)) {
  ...
  return { ...value, measurements, landmarks, calibrations, landmarkModelRefs, analysisReview, changeLog, hierarchy, cameraView, dental3d, hierarchyRefs, specimens, specimenRecords, developmentRecords: normalizeDevelopmentRecords(developmentRecords) };
}
...
export function initExtendedFeatures({ state, bones, saveLocal, selectBone, renderList, renderStats, downloadFile, listProjects, loadProject, commitInventoryEdit, applyProjectData, getModelManifest = () => null }) {
  ...
  document.querySelector('#record-panel-button').onclick = () => {
    ...
    const hierarchy = deriveHierarchy(state);
    state.hierarchy = hierarchy;
    const hierarchyLabels = state.language === 'en' ? { sites: 'Site', campaigns: 'Campaign', sectors: 'Sector', contexts: 'Context', individuals: 'Individual' } : { sites: 'Yacimiento', campaigns: 'Campaña', sectors: 'Sector', contexts: 'Contexto', individuals: 'Individuo' };
    const hierarchyFields = [['siteId', 'sites'], ['campaignId', 'campaigns'], ['sectorId', 'sectors'], ['contextId', 'contexts'], ['individualId', 'individuals']].map(([field, level]) => `<label>${hierarchyLabels[level]}<select data-hierarchy-ref="${field}"><option value="">${state.language === 'en' ? 'Not assigned' : 'Sin asociar'}</option>${(hierarchy[level] || []).map(entity => `<option value="${escapeHtml(entity.id)}">${escapeHtml(entity.name)} · ${escapeHtml(entity.id)}</option>`).join('')}</select></label>`).join('');
    const localizeScientificRecordUi = () => {
      if (state.language !== 'en') return;
      const labels = { 'record-fragments': 'Fragments', 'record-weight': 'Weight', 'record-portion': 'Anatomical portion', 'record-individual': 'Individual', 'record-taphonomy': 'Taphonomy', 'record-pathology': 'Pathology / trauma', 'record-note': 'Scientific note', 'record-taphonomy-type': 'Type', 'record-taphonomy-description': 'Description', 'record-taphonomy-position': 'Position', 'record-taphonomy-extent': 'Extent', 'record-taphonomy-evidence': 'Evidence / source', 'record-taphonomy-observations': 'Observations', 'record-pathology-type': 'State', 'record-pathology-description': 'Description', 'record-pathology-position': 'Position', 'record-pathology-extent': 'Extent', 'record-pathology-observations': 'Observations' };
      for (const [id, label] of Object.entries(labels)) { const control = document.querySelector(`#${id}`); const parent = control?.closest('label'); if (parent?.firstChild) parent.firstChild.textContent = `${label} `; }
      document.querySelectorAll('.structured-observation legend').forEach((legend, index) => { legend.textContent = index === 0 ? 'Taphonomy detail' : 'Pathology / trauma detail'; });
      const save = document.querySelector('#save-record'); if (save) save.textContent = 'Save record';
    ...
    document.querySelector('#save-record').onclick = async () => {
      ...
      const taphonomyDetail = { type: document.querySelector('#record-taphonomy-type')?.value || TAPHONOMY_OPTIONS[0], description: document.querySelector('#record-taphonomy-description')?.value.trim() || '', position: document.querySelector('#record-taphonomy-position')?.value.trim() || '', extent: document.querySelector('#record-taphonomy-extent')?.value.trim() || '', evidence: document.querySelector('#record-taphonomy-evidence')?.value.trim() || '', observations: document.querySelector('#record-taphonomy-observations')?.value.trim() || '' };
  ...
  document.querySelector('#analysis-panel-button').onclick = () => {
    ...
    const staleReviews = ['nisp', 'mne', 'mni'].filter(key => analysis[key].reviewStatus === 'stale');
    const reviewNotice = staleReviews.length ? `<p class="info-box" role="status">${state.language === 'en' ? `Manual review outdated for: ${staleReviews.map(key => key.toUpperCase()).join(', ')}. The provisional automatic value is shown until the review is updated.` : `Revisión manual obsoleta para: ${staleReviews.map(key => key.toUpperCase()).join(', ')}. Se muestra el valor automático provisional hasta actualizarla.`}</p>` : '';
    ...
    show(`${reviewNotice}<div class="analysis-summary"><div><strong>${analysis.nisp.value}</strong><small>NISP${analysis.nisp.reviewStatus === 'current' ? ' · revisado' : ''}</small></div><div><strong>${analysis.mne.value}</strong><small>MNE${analysis.mne.reviewStatus === 'current' ? ' · revisado' : ''}</small></div><div><strong>${analysis.mni.value}</strong><small>MNI${analysis.mni.reviewStatus === 'current' ? ' · revisado' : ''}</small></div><div><strong>${analysis.individuals.value}</strong><small>Individuos</small></div></div><dl class="analysis-method"><dt>NISP</dt><dd>${escapeHtml(analysis.nisp.method)}</dd><dt>MNE</dt><dd>${escapeHtml(analysis.mne.method)}</dd><dt>MNI</dt><dd>${escapeHtml(analysis.mni.method)}</dd><dt>Individuos</dt><dd>${escapeHtml(analysis.individuals.method)}</dd></dl><table class="analysis-table"><thead><tr><th>Individuo</th><th>NISP</th><th>Fragmentos</th><th>Elementos</th></tr></thead><tbody>${individualRows}</tbody></table><button id="export-analysis" class="secondary-action">Exportar análisis JSON</button>`);
    const coverage = analysis.specimenCoverage;
    const specimenNote = document.createElement('p');
    specimenNote.className = 'info-box';
    const english = state.language === 'en';
    specimenNote.textContent = english ? `Specimen traceability: ${coverage.withSpecimenId} of ${coverage.identified} identified records have Specimen_ID (${coverage.uniqueSpecimenIds} unique IDs); ${coverage.withoutSpecimenId} remain provisional.` : `Trazabilidad de especímenes: ${coverage.withSpecimenId} de ${coverage.identified} registros identificados tienen Specimen_ID (${coverage.uniqueSpecimenIds} IDs únicos); ${coverage.withoutSpecimenId} siguen siendo provisionales.`;
    ...
    if (coverage.conflicts.length) {
      ...
      conflictNote.textContent = english ? `Review associations: ${coverage.conflicts.map(conflict => conflict.specimenId).join(', ')} appears with more than one individual or context. NISP does not resolve this conflict automatically.` : `Revisar asociaciones: ${coverage.conflicts.map(conflict => conflict.specimenId).join(', ')} aparece con más de un individuo o contexto. El NISP no resuelve este conflicto automáticamente.`;
      document.querySelector('#extended-panel')?.prepend(conflictNote);
    }
    const specimenRows = Object.values(state.specimenRecords || {}).map(record => `<li><strong>${escapeHtml(record.id)}</strong>${record.label ? ` · ${escapeHtml(record.label)}` : ''}${record.individualId ? ` · ${escapeHtml(record.individualId)}` : ''}${record.context ? ` · ${escapeHtml(record.context)}` : ''}<button type="button" class="secondary-action specimen-delete" data-specimen-id="${escapeHtml(record.id)}">${english ? 'Delete record' : 'Eliminar ficha'}</button></li>`).join('') || `<li class="small-copy">${english ? 'No explicit records yet.' : 'No hay fichas explícitas todavía.'}</li>`;
    document.querySelector('#extended-panel')?.insertAdjacentHTML('afterbegin', `<details class="specimen-registry"><summary>${english ? 'Explicit specimen records' : 'Fichas explícitas de especímenes'}</summary><p class="small-copy">${english ? 'Reusable catalogue for documenting a specimen and relating it to several elements. It does not replace archaeological review.' : 'Catálogo reutilizable para documentar un espécimen y relacionarlo con varios elementos. No sustituye la revisión arqueológica.'}</p><div class="record-editor"><label>Specimen_ID<input id="specimen-record-id" placeholder="SP-001" required></label><label>${english ? 'Label' : 'Etiqueta'}<input id="specimen-record-label" placeholder="${english ? 'Specimen 001' : 'Ejemplar 001'}"></label><label>${english ? 'Individual' : 'Individuo'}<input id="specimen-record-individual" placeholder="IND-001"></label><label>${english ? 'Context' : 'Contexto'}<input id="specimen-record-context" placeholder="UE-01"></label><label>${english ? 'Notes' : 'Notas'}<textarea id="specimen-record-notes" rows="2"></textarea></label><button id="save-specimen-record" class="secondary-action" type="button">${english ? 'Save record' : 'Guardar ficha'}</button></div><ul class="specimen-record-list">${specimenRows}</ul></details>`);
  ...
  document.querySelector('#multi-compare-panel-button').onclick = async () => {
    ...
    let listedProjects;
    try {
      listedProjects = await listProjects();
    } catch (error) {
      show(`<h3>${en ? 'Compare multiple inventories' : 'Comparar varios inventarios'}</h3><p class="small-copy" role="alert">${en ? 'Local projects could not be read. Check local storage and try again.' : 'No se pudieron leer los proyectos locales. Comprueba el almacenamiento local e inténtalo de nuevo.'}</p>`);
      return;
    }
    // Always offer the active in-memory project. It may contain edits that
    // have not been flushed yet, and some fallback stores omit the active
    // record while IndexedDB is being upgraded.
    const activeProject = { ...state, id: state.projectId, projectName: state.projectName };
    const storedProjects = Array.isArray(listedProjects) ? listedProjects : [];
    const candidates = [activeProject, ...storedProjects.filter(project => project.id !== state.projectId)];
    ...
    document.querySelector('#run-multi-compare').onclick = async () => {
      ...
      const projects = (await Promise.all(ids.map(id => id === state.projectId ? activeProject : loadProject(id)))).filter(Boolean);
      ...
      const sourceLabels = projects.map(project => project.projectName || project.id);
      const rows = bones.map(bone => { const values = projects.map(project => project.status?.[bone.id] || 'not_recorded'); const counts = Object.fromEntries(statuses.map(status => [status, values.filter(value => value === status).length])); return { bone, values, counts }; }).filter(row => Object.values(row.counts).some(count => count > 0 && count < projects.length));
      ...
      setTimeout(() => { if (!result) return; result.insertAdjacentHTML('beforeend', `<button id="export-multi-compare" class="secondary-action">${en ? 'Download CSV' : 'Descargar CSV'}</button>`); document.querySelector('#export-multi-compare').onclick = () => { const fields = ['Bone_ID', ...sourceLabels, ...statuses]; const csv = [fields, ...rows.map(row => [row.bone.id, ...row.values, ...statuses.map(status => row.counts[status])])].map(line => line.map(value => `"${String(value).replaceAll('"', '""')}"`).join(',')).join('\n'); downloadFile('osteo3d-multi-comparison.csv', csv, 'text/csv;charset=utf-8'); }; }, 0);
  ...
  document.querySelector('#learning-panel-button').onclick = () => {
    ...
    const updateLearningProgress = () => {
      ...
      if (progress) progress.textContent = state.language === 'en' ? `Score: ${learningScore}/${learningAttempts}` : `Aciertos: ${learningScore}/${learningAttempts}`;
    };
    const nextQuestion = () => {
      const en = state.language === 'en';
      ...
      document.querySelectorAll('[data-answer]').forEach(button => button.onclick = () => {
        const correct = button.dataset.answer === bone.id;
        learningAttempts += 1;
        if (correct) learningScore += 1;
        document.querySelector('#quiz-feedback').textContent = correct ? (en ? 'Correct' : 'Correcto') : `${en ? 'Incorrect. Correct answer' : 'Incorrecto. Respuesta correcta'}: ${en ? bone.en : bone.es}`;
        updateLearningProgress();
      });
      document.querySelector('#next-question').onclick = nextQuestion;
      document.querySelector('#close-learning').onclick = () => { learningMode = 'identify'; learningTargetId = ''; learningSession = { mode: 'identify', targetBoneId: '' }; panel.hidden = true; document.querySelector('#details').hidden = false; };
    };
    document.querySelector('#details').hidden = true;
    nextQuestion();
  };
  window.addEventListener('oste3d:bone-selected', event => {
    if (learningMode !== 'locate' || !learningTargetId) return;
    const feedback = document.querySelector('#quiz-feedback');
    if (!feedback) return;
    const correct = event.detail?.boneId === learningTargetId;
    learningAttempts += 1;
    if (correct) learningScore += 1;
    const target = bones.find(item => item.id === learningTargetId);
    const en = state.language === 'en';
    feedback.textContent = correct ? (en ? 'Correct location' : 'Localización correcta') : `${en ? 'Not this element. Find' : 'No es este elemento. Busca'} ${en ? target?.en : target?.es}`;
    const progress = document.querySelector('#quiz-progress');
    if (progress) progress.textContent = en ? `Score: ${learningScore}/${learningAttempts}` : `Aciertos: ${learningScore}/${learningAttempts}`;
  });
        ...
        try {
          ...
          if (mergeRows) {
            ...
            for (const model of archiveModels) {
              ...
              try { await cacheCustomModelFile(profileId, boneId, fileModel); } catch { state.customModels[profileId][boneId] = { ...metadata, cached: false }; }
            }
            window.dispatchEvent(new CustomEvent('oste3d:custom-models-changed'));
          }
  ...
  document.querySelector('#changes-panel-button').onclick = () => {
    ...
    const render = () => {
      ...
      const headers = english ? ['Date', 'Bone_ID', 'Change', 'Individual', 'Investigator', 'Method'] : ['Fecha', 'Bone_ID', 'Cambio', 'Individuo', 'Investigador', 'Método'];
      const table = rows ? `<table class="analysis-table"><thead><tr>${headers.map(header => `<th>${header}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table>` : `<p class="small-copy">${english ? 'No matching changes.' : 'No hay cambios que coincidan.'}</p>`;
      const host = document.querySelector('#changes-results');
      if (host) host.innerHTML = `<p class="small-copy">${english ? `${filtered.length} entries match; showing ${visible.length}. The complete log remains in IndexedDB and backups.` : `${filtered.length} entradas coinciden; se muestran ${visible.length}. El registro completo se conserva en IndexedDB y en las copias de seguridad.`}</p>${table}`;
      const exportButton = document.querySelector('#export-change-log');
      if (exportButton) exportButton.disabled = !filtered.length;
    };
    show(`<h3>${english ? 'Change log' : 'Registro de cambios'}</h3><div class="change-log-filters"><label>${english ? 'Method' : 'Método'}<select id="change-method"><option value="all">${english ? 'All' : 'Todos'}</option>${methods.map(method => `<option value="${escapeHtml(method)}">${escapeHtml(method)}</option>`).join('')}</select></label><label>${english ? 'Search' : 'Buscar'}<input id="change-query" type="search" placeholder="${english ? 'Bone_ID, individual or investigator' : 'Bone_ID, individuo o investigador'}"></label><button id="export-change-log" class="secondary-action" type="button">${english ? 'Download CSV' : 'Descargar CSV'}</button></div><div id="changes-results"></div>`);
  ...
  document.querySelector('#hierarchy-panel-button').onclick = () => {
    ...
    const inventoryMetrics = hierarchyInventoryMetrics(hierarchy, state.hierarchyRefs, state.status);
    ...
    const filterNode = (node, level, query) => { const children = node.children.map(child => filterNode(child, level, query)).filter(Boolean); const matchesLevel = level === 'all' || node.level === level; const matchesQuery = !query || `${node.name} ${node.id}`.toLocaleLowerCase('es').includes(query); return (matchesLevel && matchesQuery) || children.length ? { ...node, children } : null; };
    const filteredTree = (level = 'all', query = '') => tree.map(node => filterNode(node, level, query)).filter(Boolean);
    const treeMarkup = (items) => items.length ? `<ul class="hierarchy-tree">${items.map(renderNode).join('')}</ul>` : `<p class="small-copy">${state.language === 'en' ? 'No matching entities.' : 'No hay entidades coincidentes.'}</p>`;
    const rows = treeMarkup(tree);
    const counts = HIERARCHY_LEVELS.map(level => `${levelLabels[level]}: ${hierarchy[level].length}`).join(' · ');
    const metricEntities = ['campaigns', 'contexts', 'individuals'].flatMap(level => hierarchy[level].map(entity => ({ level, entity, metric: inventoryMetrics[level][entity.id] })));
    const metricRows = metricEntities.map(({ level, entity, metric }) => { const reviewedRate = metric.records ? Math.round(metric.reviewed / metric.records * 100) : 0; const presentRate = metric.records ? Math.round(metric.present / metric.records * 100) : 0; return `<tr><td>${levelLabels[level]}</td><td>${escapeHtml(entity.name)}</td><td>${metric.records}</td><td>${metric.reviewed} (${reviewedRate}%)</td><td>${metric.present} (${presentRate}%)</td></tr>`; }).join('');
    const campaignEntities = metricEntities.filter(item => item.level === 'campaigns');
    const campaignOptions = campaignEntities.map(({ entity }) => `<option value="${escapeHtml(entity.id)}">${escapeHtml(entity.name)}</option>`).join('');
    const campaignComparisonTable = campaignEntities.length > 1 ? `<div class="hierarchy-campaign-comparison"><h4>${state.language === 'en' ? 'Campaign comparison' : 'Comparación entre campañas'}</h4><label>${state.language === 'en' ? 'Baseline campaign' : 'Campaña base'}<select id="hierarchy-baseline-campaign">${campaignOptions}</select></label><table class="analysis-table"><thead><tr><th>${state.language === 'en' ? 'Campaign' : 'Campaña'}</th><th>Δ ${state.language === 'en' ? 'records' : 'registros'}</th><th>Δ ${state.language === 'en' ? 'reviewed' : 'revisados'}</th><th>Δ ${state.language === 'en' ? 'present' : 'presentes'}</th></tr></thead><tbody id="hierarchy-campaign-comparison"></tbody></table><p class="small-copy">${state.language === 'en' ? 'Descriptive differences from the selected baseline; not MNE/MNI estimates.' : 'Diferencias descriptivas respecto a la base seleccionada; no son estimaciones de MNE/MNI.'}</p></div>` : '';
    const metricTable = metricRows ? `<h4>${state.language === 'en' ? 'Inventory coverage by collection entity' : 'Cobertura del inventario por entidad'}</h4><p class="small-copy">${state.language === 'en' ? 'Counts use only explicit record associations; no missing association is inferred.' : 'Los recuentos usan solo asociaciones explícitas; no se infiere ninguna asociación ausente.'}</p><button id="export-hierarchy-metrics" class="secondary-action" type="button">${state.language === 'en' ? 'Download coverage CSV' : 'Descargar cobertura CSV'}</button><table class="analysis-table hierarchy-metrics"><thead><tr><th>${state.language === 'en' ? 'Level' : 'Nivel'}</th><th>${state.language === 'en' ? 'Entity' : 'Entidad'}</th><th>${state.language === 'en' ? 'Records' : 'Registros'}</th><th>${state.language === 'en' ? 'Reviewed' : 'Revisados'}</th><th>${state.language === 'en' ? 'Present' : 'Presentes'}</th></tr></thead><tbody>${metricRows}</tbody></table>${campaignComparisonTable}` : '';
    show(`<h3>${state.language === 'en' ? 'Project hierarchy' : 'Jerarquía del proyecto'}</h3><p class="small-copy">${state.language === 'en' ? 'Relational view of site → campaign → sector → context → individual. Add entities without deleting existing observations. Stable IDs remain available for imports and comparisons.' : 'Vista relacional de yacimiento → campaña → sector → contexto → individuo. Añade entidades sin borrar observaciones existentes. Los IDs estables siguen disponibles para importaciones y comparaciones.'}</p><p class="hierarchy-counts">${escapeHtml(counts)}</p><div class="record-editor"><label>${state.language === 'en' ? 'Level' : 'Nivel'}<select id="hierarchy-level">${options}</select></label><label>${state.language === 'en' ? 'Name' : 'Nombre'}<input id="hierarchy-name" placeholder="${state.language === 'en' ? 'e.g. Site North' : 'p. ej. Yacimiento Norte'}"></label><label>${state.language === 'en' ? 'Parent ID (optional)' : 'ID padre (opcional)'}<input id="hierarchy-parent" placeholder="site:yacimiento-norte"></label><button id="add-hierarchy-entity" class="secondary-action">${state.language === 'en' ? 'Add entity' : 'Añadir entidad'}</button><p id="hierarchy-message" role="status"></p></div><div class="hierarchy-filter"><label>${state.language === 'en' ? 'Show level' : 'Mostrar nivel'}<select id="hierarchy-filter-level"><option value="all">${state.language === 'en' ? 'All levels' : 'Todos los niveles'}</option>${options}</select></label><label>${state.language === 'en' ? 'Search hierarchy' : 'Buscar en jerarquía'}<input id="hierarchy-filter-query" type="search" placeholder="${state.language === 'en' ? 'Name or stable ID' : 'Nombre o ID estable'}"></label></div><div id="hierarchy-tree-view" class="hierarchy-grid">${rows}</div>${metricTable}`);
    ...
    document.querySelector('#export-hierarchy-metrics')?.addEventListener('click', () => {
      const fields = ['Level', 'Entity_ID', 'Entity', 'Records', 'Reviewed', 'Reviewed_pct', 'Present', 'Present_pct'];
      const csvCell = value => `"${String(value ?? '').replaceAll('"', '""')}"`;
      const csvRows = metricEntities.map(({ level, entity, metric }) => [level, entity.id, entity.name, metric.records, metric.reviewed, metric.records ? Math.round(metric.reviewed / metric.records * 100) : 0, metric.present, metric.records ? Math.round(metric.present / metric.records * 100) : 0].map(csvCell).join(','));
      downloadFile('osteo3d-hierarchy-coverage.csv', [fields.map(csvCell).join(','), ...csvRows].join('\r\n'), 'text/csv;charset=utf-8');
    });
    const parentInput = document.querySelector('#hierarchy-parent');
    if (parentInput) {
      const parentSelect = document.createElement('select');
      parentSelect.id = 'hierarchy-parent';
      parentSelect.setAttribute('aria-label', state.language === 'en' ? 'Parent ID (optional)' : 'ID padre (opcional)');
      const parentLevels = { campaigns: ['sites'], sectors: ['campaigns'], contexts: ['sectors'], individuals: ['contexts'] };
      const syncParentOptions = () => { const level = document.querySelector('#hierarchy-level').value; const allowed = parentLevels[level] || []; parentSelect.innerHTML = `<option value="">${state.language === 'en' ? 'No parent' : 'Sin entidad padre'}</option>` + allowed.flatMap(parentLevel => hierarchy[parentLevel].map(entity => `<option value="${escapeHtml(entity.id)}">${escapeHtml(entity.name)} · ${escapeHtml(entity.id)}</option>`)).join(''); };
      parentInput.replaceWith(parentSelect);
      document.querySelector('#hierarchy-level').addEventListener('change', syncParentOptions);
      syncParentOptions();
    }
  ...
  const sourcePanelMarkup = () => state.language === 'en'
    ? '<h3>Sources and licenses</h3><p>The adult male profile publishes 179 of 192 GLB elements; the remaining 13 are aggregate or indeterminate inventory categories. The skull source is CC0 (<a href="https://creativecommons.org/public-domain/cc0/" target="_blank" rel="noopener noreferrer">legal text</a>), while the other assets and adaptations retain CC BY-SA 4.0. Adult female, infant and neonate publish 179 Blender-adapted GLBs per profile (537 files), derived from the adult-male package under CC BY-SA 4.0. These are didactic visual adaptations without metric scale or anatomical validation. The application code is MIT (<a href="https://opensource.org/license/mit/" target="_blank" rel="noopener noreferrer">license</a>).</p><p class="small-copy"><a href="./models/SOURCES.md" target="_blank" rel="noopener">Model sources</a> · <a href="./LICENSE" target="_blank" rel="noopener noreferrer">Code license</a> · <a href="./THIRD_PARTY_LICENSES.md" target="_blank" rel="noopener noreferrer">Third-party licenses</a>.</p>'
    : '<h3>Fuentes y licencias</h3><p>El perfil adulto masculino publica 179 de 192 elementos GLB; los 13 restantes son categorías agregadas o indeterminadas del inventario. El cráneo procede de una fuente CC0 (<a href="https://creativecommons.org/public-domain/cc0/" target="_blank" rel="noopener noreferrer">texto legal</a>); los demás assets y adaptaciones mantienen CC BY-SA 4.0. Adulto femenino, infante y neonato publican 179 GLB adaptados con Blender por perfil (537 archivos), derivados del paquete masculino bajo CC BY-SA 4.0. Son adaptaciones visuales didácticas, sin escala métrica ni validación anatómica. El código de la aplicación es MIT (<a href="https://opensource.org/license/mit/" target="_blank" rel="noopener noreferrer">licencia</a>).</p><p class="small-copy"><a href="./models/SOURCES.md" target="_blank" rel="noopener">Fuentes de modelos</a> · <a href="./LICENSE" target="_blank" rel="noopener noreferrer">Licencia del código</a> · <a href="./THIRD_PARTY_LICENSES.md" target="_blank" rel="noopener noreferrer">Licencias de terceros</a>.</p>';
  document.querySelector('#sources-panel-button').onclick = () => show(sourcePanelMarkup());
