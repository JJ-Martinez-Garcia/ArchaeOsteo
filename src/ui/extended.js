...
export function initExtendedFeatures({ state, bones, saveLocal, selectBone, renderList, renderStats, downloadFile, listProjects, loadProject }) {
  ...
  document.querySelector('#analysis-panel-button').onclick = () => {
    ...
    document.querySelector('#export-analysis').insertAdjacentHTML('beforebegin', `<h4>NISP por contexto</h4><table class="analysis-table"><thead><tr><th>Contexto / UE</th><th>NISP</th></tr></thead><tbody>${contextRows}</tbody></table>`);
  };
  ...
  document.querySelector('#sources-panel-button').onclick = () => show('<h3>Fuentes y licencias</h3><p>Los marcadores geométricos restantes son material de desarrollo y no representan modelos anatómicos aptos para medición o diagnóstico.</p><p>El perfil adulto masculino incluye parcialmente el cráneo <code>skull.glb</code>, de Vladimir Petkovic, procedente de Khronos glTF Sample Assets y distribuido bajo CC0 1.0 Universal. Se conserva la atribución por trazabilidad.</p><p>Los perfiles adulto femenino, infante y neonato, y los demás huesos del adulto masculino, permanecen pendientes de incorporar como GLB independientes con correspondencia verificada con cada <code>Bone_ID</code>.</p><p class="small-copy">El registro completo está en <a href="https://github.com/JJ-Martinez-Garcia/ArchaeOsteo/blob/main/public/models/SOURCES.md" target="_blank" rel="noreferrer">public/models/SOURCES.md</a>.</p>');
  document.querySelector('#indeterminate-fragments-button').onclick = () => {
    const render = () => {
      const rows = state.indeterminateFragments.map((item, index) => `<li><strong>${escapeHtml(item.type || 'Sin tipo')}</strong> · ${escapeHtml(item.size || 'Tamaño no registrado')} · ${item.quantity || 1} ud. · ${item.weight == null ? 'Peso no registrado' : `${item.weight} g`}<small>${item.length == null ? 'Longitud —' : `L ${item.length} mm`} · ${item.width == null ? 'Anchura —' : `A ${item.width} mm`} · ${item.thickness == null ? 'Espesor —' : `E ${item.thickness} mm`}<br>${escapeHtml(item.individual || 'Sin individuo')} · ${escapeHtml(item.context || 'Sin contexto')} · ${escapeHtml(item.observations || 'Sin observaciones')}</small><button class="secondary-action" data-remove-indeterminate="${index}">Eliminar</button></li>`).join('');
      ...
      document.querySelector('#add-indeterminate').onclick = async () => { const weight = Number(document.querySelector('#indeterminate-weight').value); const quantity = Number(document.querySelector('#indeterminate-quantity').value); const length = Number(document.querySelector('#indeterminate-length').value); const width = Number(document.querySelector('#indeterminate-width').value); const thickness = Number(document.querySelector('#indeterminate-thickness').value); state.indeterminateFragments.push({ type: document.querySelector('#indeterminate-type').value.trim(), size: document.querySelector('#indeterminate-size').value.trim(), quantity: Number.isFinite(quantity) && quantity >= 1 ? Math.floor(quantity) : 1, weight: Number.isFinite(weight) && weight >= 0 ? weight : null, length: Number.isFinite(length) && length >= 0 ? length : null, width: Number.isFinite(width) && width >= 0 ? width : null, thickness: Number.isFinite(thickness) && thickness >= 0 ? thickness : null, individual: document.querySelector('#indeterminate-individual').value.trim(), context: document.querySelector('#indeterminate-context').value.trim(), observations: document.querySelector('#indeterminate-observations').value.trim(), createdAt: new Date().toISOString() }); await saveLocal({ notify: false }); render(); };
