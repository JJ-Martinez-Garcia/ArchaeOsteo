...
export function initExtendedFeatures({ state, bones, saveLocal, selectBone, renderList, renderStats, downloadFile, listProjects, loadProject }) {
  ...
  document.querySelector('#record-panel-button').onclick = () => {
    ...
    document.querySelector('#save-record').onclick = async () => {
      ...
      state.taphonomyDetails[id] = { type: document.querySelector('#record-taphonomy-type')?.value || TAPHONOMY_OPTIONS[0], description: document.querySelector('#record-taphonomy-description')?.value.trim() || '', position: document.querySelector('#record-taphonomy-position')?.value.trim() || '', extent: document.querySelector('#record-taphonomy-extent')?.value.trim() || '', observations: document.querySelector('#record-taphonomy-observations')?.value.trim() || '' };
