...
export function initExtendedFeatures({ state, bones, saveLocal, selectBone, renderList, renderStats, downloadFile, listProjects, loadProject, commitInventoryEdit, applyProjectData }) {
  ...
  document.querySelector('#hierarchy-panel-button').onclick = () => {
    ...
    if (parentInput) { const parentSelect = document.createElement('select'); parentSelect.id = 'hierarchy-parent'; parentSelect.setAttribute('aria-label', state.language === 'en' ? 'Parent ID (optional)' : 'ID padre (opcional)'); parentSelect.innerHTML = `<option value="">${state.language === 'en' ? 'No parent' : 'Sin entidad padre'}</option>` + HIERARCHY_LEVELS.flatMap(level => hierarchy[level].map(entity => `<option value="${escapeHtml(entity.id)}">${escapeHtml(entity.name)} · ${escapeHtml(entity.id)}</option>`)).join(''); parentInput.replaceWith(parentSelect); }
