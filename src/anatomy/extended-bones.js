...
for (const side of [['left', 'Izquierda', 'Left', -1], ['right', 'Derecha', 'Right', 1]]) {
  for (let i = 1; i <= 12; i += 1) {
    const y = 1.9 - (i - 1) * .18;
    add(`${side[0]}_rib_${i}`, `Costilla ${i} ${side[1].toLowerCase()}`, `${side[2]} rib ${i}`, `Costa ${side[0] === 'left' ? 'sinistra' : 'dextra'} ${i}`, 'Tórax', side[1], [side[3] * (.62 + i * .015), y, -.12], [side[3] * (2.55 + (i % 3) * .18), 2.2 - i * .16, 0], [1.05 - i * .025, .055, .055], 'rib');
  }
}
...
for (const side of [['left', 'Izquierda', 'Left', -1], ['right', 'Derecha', 'Right', 1]]) {
  const prefix = side[0]; const sign = side[3];
  add(`${prefix}_scapula`, `Escápula ${side[1].toLowerCase()}`, `${side[2]} scapula`, `Scapula ${prefix === 'left' ? 'sinistra' : 'dextra'}`, 'Cintura escapular', side[1], [sign * 1.15, 1.35, -.12], [sign * 3.0, 2.6, 0], [.45, .7, .1], 'flat_bone');
  add(`${prefix}_radius`, `Radio ${side[1].toLowerCase()}`, `${side[2]} radius`, `Radius ${prefix === 'left' ? 'sinister' : 'dexter'}`, 'Extremidad superior', side[1], [sign * 1.32, -.55, 0], [sign * 3.2, .5, 0], [.14, 1.7, .14], 'long_bone');
  add(`${prefix}_ulna`, `Ulna ${side[1].toLowerCase()}`, `${side[2]} ulna`, `Ulna ${prefix === 'left' ? 'sinistra' : 'dextra'}`, 'Extremidad superior', side[1], [sign * 1.55, -.55, -.12], [sign * 3.55, .5, 0], [.14, 1.75, .14], 'long_bone');
  add(`${prefix}_patella`, `Patela ${side[1].toLowerCase()}`, `${side[2]} patella`, `Patella ${prefix === 'left' ? 'sinistra' : 'dextra'}`, 'Extremidad inferior', side[1], [sign * .5, -2.72, .35], [sign * 2.0, -2.0, 0], [.22, .28, .1], 'sesamoid');
  add(`${prefix}_fibula`, `Fíbula ${side[1].toLowerCase()}`, `${side[2]} fibula`, `Fibula ${prefix === 'left' ? 'sinistra' : 'dextra'}`, 'Extremidad inferior', side[1], [sign * .76, -3.4, -.1], [sign * 2.3, -3.9, 0], [.12, 2.3, .12], 'long_bone');
}
...
for (const side of [['left', 'Izquierda', 'Left', -1], ['right', 'Derecha', 'Right', 1]]) {
  const [prefix, label, englishLabel, sign] = side;
  carpals.forEach(([id, name], i) => add(`${prefix}_${id}`, `${name} ${label.toLowerCase()}`, `${englishLabel} ${id}`, `${name} ${prefix === 'left' ? 'sinister' : 'dexter'}`, 'Manos', label, [sign * 1.7 + (i % 4) * .12, -.75 - Math.floor(i / 4) * .14, .08], [sign * 4.0, 1.2 - i * .14, 0], [.12, .12, .1], 'carpal'));
  ...
  for (let i = 1; i <= 5; i += 1) {
    add(`${prefix}_metacarpal_${i}`, `Metacarpiano ${i} ${label.toLowerCase()}`, `${englishLabel} metacarpal ${i}`, `Os metacarpale ${i}`, 'Manos', label, [sign * 1.95 + (i - 3) * .13, -1.15, 0], [sign * 4.5, .45 - i * .13, 0], [.1, .65, .1], 'metacarpal');
    for (const [part, partName, scale] of [['proximal', 'proximal', .23], ['middle', 'media', .18], ['distal', 'distal', .16]]) {
      if (i === 1 && part === 'middle') continue;
      add(`${prefix}_digit_${i}_${part}`, `Falange ${i} ${partName} ${label.toLowerCase()}`, `${englishLabel} digit ${i} ${partName}`, `Phalanx ${partName}`, 'Manos', label, [sign * (2.05 + (i - 3) * .13), -1.85 - (part === 'proximal' ? 0 : .28), 0], [sign * 4.9, .3 - i * .12 - (part === 'distal' ? .35 : 0), 0], [.09, scale, .09], 'phalanx');
    }
  }
  if (prefix === 'left') {
  ...
  tarsals.forEach(([id, name], i) => add(`${prefix}_${id}`, `${name} ${label.toLowerCase()}`, `${englishLabel} ${id}`, `${name} ${prefix === 'left' ? 'sinister' : 'dexter'}`, 'Pies', label, [sign * 1.65 + (i % 3) * .15, -3.85 - Math.floor(i / 3) * .14, .05], [sign * 4.0, -2.0 - i * .14, 0], [.16, .16, .13], 'tarsal'));
  for (let i = 1; i <= 5; i += 1) {
    add(`${prefix}_metatarsal_${i}`, `Metatarsiano ${i} ${label.toLowerCase()}`, `${englishLabel} metatarsal ${i}`, `Os metatarsale ${i}`, 'Pies', label, [sign * 1.9 + (i - 3) * .15, -4.35, 0], [sign * 4.5, -2.2 - i * .12, 0], [.11, .72, .11], 'metatarsal');
    for (const [part, partName, scale] of [['proximal', 'proximal', .24], ['middle', 'media', .18], ['distal', 'distal', .15]]) {
      if (i === 1 && part === 'middle') continue;
      add(`${prefix}_toe_${i}_${part}`, `Falange del dedo ${i} ${partName} ${label.toLowerCase()}`, `${englishLabel} toe ${i} ${partName}`, `Phalanx ${partName}`, 'Pies', label, [sign * (2.0 + (i - 3) * .15), -5.05 - (part === 'proximal' ? 0 : .25), 0], [sign * 5.0, -2.5 - i * .1 - (part === 'distal' ? .3 : 0), 0], [.09, scale, .09], 'phalanx');
    }
  }
  if (prefix === 'left') {
