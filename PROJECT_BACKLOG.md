...
Actualizado: 2026-09-11. Fuente: los 119 apartados del encargo original.
...
| 22 | MODO APRENDIZAJE | Preguntas por niveles y modos identificar/localizar; la ficha se oculta durante el ejercicio, se restaura al salir y la sesión ofrece feedback y puntuación de intentos. E2E verifica el flujo; queda validación pedagógica del banco. |
...
| 26 | CONSERVACIÓN | Distingue `not_evaluated` (no evaluada), `not_evaluable` (no evaluable) y porcentaje no registrado (`—`). La normalización y las acciones de pintura eliminan porcentajes inconsistentes cuando un elemento es no evaluable; las medias solo usan porcentajes observados. Queda ampliar validación visual y con colecciones reales. |
...
| 29 | LATERALIDAD | Filtros, normalización y resumen distinguen izquierda, derecha, `indeterminate` (indeterminada) y `not_applicable` (no aplicable); `—` se conserva como alias compatible de no aplicable. Queda validar colecciones reales con códigos laterales no estándar. |
...
| 35 | INFANTES Y NEONATOS | Registro independiente de epífisis/centros/componentes con relaciones de fusión y estados observados por componente, persistencia, deshacer y XLSX importable; catálogo orientativo de osificación con fuentes integrado sin inferencia automática de edad. Queda validación osteológica especializada y selección de referencias aplicables a cada población. |
...
| 38 | NISP | `Specimen_ID` explícito por registro, ficha reutilizable por espécimen (etiqueta, individuo, contexto y notas), cobertura de IDs únicos, importación/exportación XLSX y alerta de conflictos implementados; el automático agrupa el mismo espécimen por elemento y conserva el carácter provisional sin ID. Sigue pendiente la validación arqueológica especializada. |
| 39 | MNE | Cálculo automático provisional y revisión manual con entero, justificación y firma del inventario filtrado; conserva límites sobre porciones/solapamiento, marca una revisión antigua como obsoleta tras cambiar los datos y exige revisión especializada. Las asociaciones jerárquicas explícitas alimentan el contexto e individuo del cálculo; la E2E comprueba el aviso tras una modificación. |
| 40 | MNI | Cálculo automático provisional y revisión manual con entero, justificación y firma del inventario filtrado; no cuenta fragmentos como individuos, marca una revisión antigua como obsoleta tras cambiar los datos y conserva los límites de asociación. Las asociaciones jerárquicas explícitas alimentan el individuo del cálculo; la E2E comprueba el aviso tras una modificación. |
| 41 | FRAGMENTOS INDETERMINADOS | Cantidades y medidas opcionales, asociación a individuo/UE/contexto y deshacer de eliminación verificados en E2E; queda ampliar la validación arqueológica de los campos. |
...
| 43 | TAFONOMÍA | Catálogo curado y anotaciones múltiples estructuradas por hueso, con campo explícito de evidencia/fuente y observaciones. |
| 44 | PATOLOGÍA Y TRAUMA | Registro estructurado sin diagnóstico automático y botón para marcar patología/trauma sobre la superficie 3D; el punto local queda enlazado en `pathologyDetails.surfaceLandmark`, se visualiza como landmark y se exporta con la ficha. |
| 45 | FOTOGRAFÍAS | Límite por archivo de 12 MB, guardia acumulada de 64 MB, edición de leyenda y asociación por ámbito (incluida patología/trauma) verificados en E2E; queda probar cuotas reales de dispositivos. |
| 46 | PINTAR INVENTARIO | Pruebas de pintura táctil en tablet/móvil y accesos rápidos, sin arrastrar la cámara accidentalmente; el canvas cancela el gesto táctil en modo inventario y no persiste movimientos de cámara al pintar. |
| 47 | BARRA DE PINTURA | Barra y estado persistente operativos; probar todos los estados con teclado/táctil. |
| 48 | FUNCIONAMIENTO DE PINTAR INVENTARIO | Secuencia de pintar varios huesos, deshacer y recargar como E2E de inventario completo; la selección múltiple y acciones masivas confirmadas ya tienen cobertura E2E. La E2E comprueba además pintar desde tabla durante despliegue/comparación y recuperar por deshacer. |
...
| 55 | PINTAR PORCENTAJE | Atajos táctiles 100/75/50/25/<25 y “Sin porcentaje” implementados como transacciones reversibles, persistentes y respetuosas con bloqueos; el valor vacío sigue siendo desconocido y no 0. |
| 56 | PINTAR ALTERACIONES | E2E de alteraciones múltiples sin sobrescritura de detalles existentes. |
| 57 | BORRADOR | Borrado de asignación limpia todos los campos científicos propios del elemento (estado, conservación, medidas, fragmentos, peso, porciones, desarrollo, jerarquía, individuo, UE, alteraciones, landmarks y anotaciones/foto) y E2E verifica su recuperación por deshacer. |
| 58 | DESHACER / REHACER | Importaciones, ficha científica, medidas, fotografías, altas/bajas de fragmentos indeterminados, filtros y cambios visuales principales usan snapshots comunes reversibles; la cámara 3D también forma parte del snapshot. Queda ampliar la auditoría a configuración restante y probar recuperación tras recarga en todos los dispositivos. |
| 59 | BLOQUEAR REGISTROS | Formulario científico, importación revalidada, tablas, medidas, landmarks, fotos y modelos personalizados cubiertos; bloqueo/desbloqueo también es reversible y queda auditado. |
| 60 | MOSTRAR PENDIENTES | El modo muestra solo registros `not_recorded` en árbol y 3D, conserva agrupaciones/etiquetas y restaura la vista completa al desactivarse; el contador anuncia los resultados pendientes. |
...
| 63 | PRESENCIA RÁPIDA | Prueba de presencia rápida y persistencia; al activarla desactiva y actualiza explícitamente el modo de fragmentación para evitar estados incompatibles. |
...
| 66 | PINTAR EN CUALQUIER DISPOSICIÓN | La E2E comprueba editar el estado durante despliegue y comparación y entrar/salir de mesa osteológica conservando el registro; queda ampliar la matriz táctil a todas las disposiciones y elementos pequeños en dispositivos reales. |
...
| 71 | TABLA DE INVENTARIO | Vacío frente a cero probado en tabla; edición continua conserva foco y selección tras refrescar la fila, con sincronización de tabla, ficha, árbol y 3D. |
| 72 | FILTROS | Filtros científicos y de representación comparten ahora el mismo subconjunto en árbol, 3D y estadísticas; la E2E verifica una combinación y su limpieza. Queda ampliar la matriz completa de combinaciones en dispositivos reales. |
...
| 74 | INFORME AUTOMÁTICO | Informe con fuentes/métodos/límites, jerarquía normalizada, observaciones de componentes inmaduros, fotos elegidas, mapa esquelético vectorial regional con patrones de estado/progreso y reglas de impresión A4 para evitar cortes; la E2E verifica la presencia del mapa y las reglas de impresión. Queda maquetación PDF avanzada y validación científica de la cartografía. |
| 75 | EXPORTACIÓN | JSON conserva la copia completa, incluida la vista de cámara, componentes inmaduros y asociaciones jerárquicas por registro; CSV contiene el inventario tabular, las porciones independientes, asociaciones y su registro inmaduro JSON; XLSX incluye e importa inventario, contexto, jerarquía, asociaciones, odontogramas, fragmentos, desarrollo, osteometría, landmarks, calibraciones, referencias, revisión, cambios y metadatos de fotos/modelos. Un fixture realista de campo verifica el round-trip de los campos científicos principales; falta contrastarlo con exportaciones de colecciones reales y decidir un formato tabular para binarios. |
| 76 | CSV/XLSX | Esquema estable y códigos canónicos, comillas/saltos de línea, trazabilidad de fuentes/método/límites y hoja `Jerarquía` con IDs padre. La cobertura de hojas auxiliares y una hoja `Esquema` autodocumentada están implementadas; el fixture de campo cubre individuos, especímenes, porciones, dientes, fragmentos, jerarquía y notas multilínea. Queda auditoría con archivos reales. |
...
| 79 | BASE DE DATOS LOCAL | Modelo de entidades explícitas en IndexedDB y migraciones sin pérdida; la migración formal a esquema de proyecto v3 incorpora el registro jerárquico normalizado además de los mapas de inventario y está probada con proyectos v2. El diagnóstico PWA muestra uso/cuota cuando el navegador lo permite; queda auditoría de cuotas en dispositivos y escenarios extremos de recuperación. |
| 80 | JERARQUÍA DE PROYECTOS | Entidades normalizadas de yacimiento/campaña/sector/contexto/individuo, IDs estables, relaciones padre, editor no destructivo, selector de entidades padre existentes y round-trip JSON/OSTEO3D/XLSX verificados. Cada registro óseo puede asociarse ahora mediante selector a los IDs de la jerarquía y la relación se conserva en CSV/XLSX, copias y deshacer. La interfaz muestra un árbol relacional anidado, recuentos de registros óseos por entidad, porcentajes de revisión/presencia y exportación CSV de cobertura; conserva como raíces las entidades importadas cuyo padre aún no existe. Queda ampliar la vista con métricas comparativas especializadas de múltiples campañas. |
...
| 82 | COPIAS DE SEGURIDAD | Copia completa con fotografías y modelos personalizados binarios, checksum CRC y SHA-256 por entrada en manifiesto y restauración verificada. El lector limita tamaño total/entrada, número de entradas y rutas duplicadas o inseguras antes de procesar; sigue pendiente probar cuotas y recuperación en dispositivos reales. |
| 83 | COMPARAR INVENTARIOS | Comparación detallada de dos proyectos locales con identidad visible de individuo/contexto/UE/campaña, filtro por esos valores y porciones independientes por elemento; además, vista agregada para dos o más fuentes (incluido el proyecto activo) con recuento por estado, exportación CSV con columna y estado de cada fuente, y E2E de dos fuentes locales. Los valores desconocidos se muestran como `—` y los ceros observados se conservan. La comparación 3D sincroniza cámara, despliegue, mesa, filtros científicos, pendientes, aislamiento, selección por clic y controles de transparencia, X-Ray, wireframe y colores por región. Queda revisión científica. |
| 84 | BASE ANATÓMICA | Catálogo modular con tipos y nomenclatura curados; source de datos separado ya iniciado. |
| 85 | INTERNACIONALIZACIÓN | ES/EN y textos principales disponibles; el informe imprimible, la auditoría, el editor científico y la preferencia de calidad de renderizado localizan títulos y etiquetas fijas sin tocar el texto científico del investigador. Quedan RTL, algunos mensajes auxiliares incrustados y nuevos idiomas. |
| 86 | PWA Y OFFLINE | PWA/offline probados en Chromium; confirmar navegación y operaciones completas tras instalación. |
| 87 | DESCARGA DE MODELOS | Paquetes de los cuatro perfiles disponibles; descarga e importación local transaccionales (revierten solo recursos añadidos y restauran reemplazos si falla), y eliminación separada de modelos personales; queda prueba en dispositivos reales. |
| 88 | OPTIMIZACIÓN 3D | Liberación de geometrías/materiales, carga concurrente limitada (2 en poca memoria, 4 en equipos normales) y cesión del hilo tras cada GLB para no saturar el render, diagnóstico visible de mallas/triángulos/origen con advertencia orientativa, selector persistente de calidad (automática/baja/alta) y LOD geométrico limitado para calidad baja implementados. Quedan presupuestos formales, validación visual del LOD y medición en móviles reales. |
| 89 | RENDIMIENTO | Matriz de PC/Mac/Android/iOS/iPad y métricas reproducibles en dispositivos reales. |
| 90 | INTERFAZ DE ESCRITORIO | Panel redimensionable y formularios probados en PC; diálogos accesibles y refrescos en segundo plano optimizados. La E2E comprueba ahora el inspector al 125 %, 150 % y 200 %; queda ampliar la matriz a todos los tamaños y plataformas. |
...
| 92 | ACCESIBILIDAD | Foco visible global, pestañas ARIA con navegación por teclado, anuncio vivo de selección, soporte de movimiento reducido, contraste AA de textos secundarios y objetivos táctiles mínimos de 44 px incorporados y auditados en E2E. Queda auditoría completa con lector de pantalla, zoom y dispositivos reales. |
| 93 | ARQUITECTURA DEL PROYECTO | Extraer controlador 3D y módulos de UI de main.js de forma incremental. |
| 94 | MODULARIDAD | Los exportadores XLSX/pesos, los controles de fotografías, los filtros extendidos y el reset de nuevo proyecto ya no dependen de temporizadores de carga arbitrarios; el diagnóstico de capacidades tampoco se inicializa dos veces y el gestor de proyectos usa una microtarea segura. Quedan los temporizadores de UI restantes y la separación completa de dependencias cruzadas. |
| 95 | LICENCIAS DE MODELOS 3D | 537 GLB adaptados con Blender y 179 externos documentados; las fuentes, autoría, licencia, URL de términos, fecha de consulta y advertencia didáctica están incorporadas y validadas. Sigue pendiente la auditoría especializada de cada licencia/fuente externa y validar anatómicamente las adaptaciones antes de cualquier uso científico. |
...
| 108 | CONTROL DE ERRORES | Validar fallos de red, modelo corrupto, cuota, migración, importación y recuperación sin falso éxito; la descarga de paquetes ya revierte parciales sin borrar entradas previas y la E2E limita las navegaciones CDP para diagnosticar bloqueos del navegador. |
...
| 113 | ACTUALIZACIONES | La actualización aplaza la recarga si el guardado devuelve `ok: false` o si el formulario del informe tiene cambios sin guardar; la E2E comprueba que el borrador no se persiste y que se muestra el aviso. Queda probar la transición completa con un Service Worker esperando y formulario sucio en dispositivos reales. |
