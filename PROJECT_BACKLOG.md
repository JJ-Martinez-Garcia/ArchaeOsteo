...
| 69 | REGISTRO DE CAMBIOS | Snapshots comunes registran también cambios de fotografías, fragmentos indeterminados, odontogramas, contexto, medidas, landmarks y calibraciones; queda cubrir operaciones visuales/configuración y mejorar la vista de auditoría. |
...
| 75 | EXPORTACIÓN | JSON conserva la copia completa; CSV contiene el inventario tabular; XLSX incluye e importa inventario, contexto, jerarquía, odontogramas, fragmentos, osteometría, landmarks, calibraciones, referencias, revisión, cambios y metadatos de fotos/modelos. Falta verificar round-trip con casos reales y decidir un formato tabular para binarios. |
...
| 79 | BASE DE DATOS LOCAL | Modelo de entidades explícitas en IndexedDB y migraciones sin pérdida; proyectos guardan ahora un registro jerárquico normalizado además de los mapas de inventario. Queda una migración formal de versión de esquema y auditoría de cuotas. |
| 80 | JERARQUÍA DE PROYECTOS | Entidades normalizadas de yacimiento/campaña/sector/contexto/individuo, IDs estables, relaciones padre, editor no destructivo y round-trip JSON/OSTEO3D/XLSX verificados. Queda una vista relacional completa para múltiples campañas y asociación de cada registro a entidades con selector, no solo derivación desde campos. |
